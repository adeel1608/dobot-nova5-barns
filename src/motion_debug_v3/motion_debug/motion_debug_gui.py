...
# V3 compatibility: Dobot ROS2 SDK V3 publishes messages and services from
# dobot_msgs_v3 instead of dobot_msgs_v4.
from dobot_msgs_v3.msg import RobotStatus, ToolVectorActual
...
QUEUED_MOTION_SERVICES = {
    # V3 compatibility: joint targets that used V4 MovJ(mode=True) are sent through V3 JointMovJ.
    f'{SERVICE_ROOT}/JointMovJ',
    f'{SERVICE_ROOT}/MovL',
}
...
def _load_robot_type_from_bringup_config() -> str:
    ...
    candidate_paths.append(Path(__file__).resolve().parents[2] / 'dobot_bringup_v3' / 'config' / 'param.json')
...
class MotionDebugNode(Node):
    def __init__(self) -> None:
        ...
        # V3 compatibility: feedback.py in DOBOT_6Axis_ROS2_V3 publishes this topic name.
        self.create_subscription(ToolVectorActual, 'dobot_msgs_v3/msg/ToolVectorActual', self._tcp_callback, 10)
        # V3 compatibility: keep the V3 RobotStatus topic if a status publisher is present.
        self.create_subscription(RobotStatus, 'dobot_msgs_v3/msg/RobotStatus', self._status_callback, 10)
        ...
        # V3 compatibility: V3 has no Stop service; ResetRobot is the closest dashboard stop/reset equivalent.
        self._reset_robot_client = self.create_client(ResetRobot, f'{SERVICE_ROOT}/ResetRobot')
        # V3 compatibility: dobot_bringup_v3 does not create StopMoveJog; MoveJog() with no axis stops jogging.
        self._move_jog_client = self.create_client(MoveJog, f'{SERVICE_ROOT}/MoveJog')
        ...
        self._speed_j_client = self.create_client(SpeedJ, f'{SERVICE_ROOT}/SpeedJ')
        self._acc_j_client = self.create_client(AccJ, f'{SERVICE_ROOT}/AccJ')
        self._speed_l_client = self.create_client(SpeedL, f'{SERVICE_ROOT}/SpeedL')
        self._acc_l_client = self.create_client(AccL, f'{SERVICE_ROOT}/AccL')
        # V3 compatibility: V4 MovJ(mode=True) maps to V3 JointMovJ for joint-space targets.
        self._joint_mov_j_client = self.create_client(JointMovJ, f'{SERVICE_ROOT}/JointMovJ')
    ...
    def toggle_enable(self) -> None:
        ...
        if snapshot.controller_mode_code == 10:
            # V3 compatibility: replace V4 Stop with V3 ResetRobot.
            self._send_simple_service(
                self._reset_robot_client,
                ResetRobot.Request(),
                'Exit Pause',
                f'{SERVICE_ROOT}/ResetRobot',
                on_success=self._enable_after_pause,
            )
            return

        if snapshot.controller_mode_code == 11:
            # V3 compatibility: replace V4 StopMoveJog with MoveJog(axis_id='', param_value=[]).
            self._send_simple_service(
                self._move_jog_client,
                self._build_stop_jog_request(),
                'Recover Jog',
                f'{SERVICE_ROOT}/MoveJog',
    ...
    def _send_tool_tcp(
        ...
        return self._send_simple_service(
            ...
            on_success=lambda sent_values=list(values): self._mark_tool_tcp(sent_values),
            allow_local_queue=allow_local_queue,
        )
    ...
    def stop_motion_script(self) -> bool:
        ...
        with self._lock:
            if not self._is_script_busy_locked():
                self._snapshot.action_text = 'No script is currently running. Sending ResetRobot...'
        ...
        if not sent:
            self._finish_script_run(
                f'Script "{active_name}" stop failed: ResetRobot service is not ready.',
                clear_pending_goals=True,
            )
            return False
        return True
    ...
    def stop_robot_motion(self, action_name: str = 'Stop Robot', on_complete=None) -> bool:
        # V3 compatibility: V4 Stop is not exposed by DOBOT_6Axis_ROS2_V3.
        # ResetRobot is the closest available dashboard command for stopping/resetting robot motion.
        sent = self._send_simple_service(
            self._reset_robot_client,
            ResetRobot.Request(),
            action_name,
        ...
        if not sent:
            self._set_action_text(f'{action_name}: ResetRobot service is not ready.')
        return sent
    ...
    def _build_movj_request(
        ...
        # V3 compatibility: V4 MovJ(mode=True, a..f) represented joint motion.
        # V3 splits that into JointMovJ(j1..j6); V3 MovJ is Cartesian x/y/z/rx/ry/rz.
    ...
    def _build_movl_request(
        ...
        # V3 compatibility: V4 MovL(mode=False, a..f) maps to V3 MovL(x/y/z/rx/ry/rz).
    ...
    def _run_script_dispatch_point_motion(
        ...
        if point.motion_type == 'MovJ':
            # V3 compatibility: script MovJ points are joint-space points, so dispatch via JointMovJ.
            client = self._joint_mov_j_client
            request = self._build_movj_request(point.values, use_tool=point.use_tool, motion_args=point.motion_args)
            service_name = f'{SERVICE_ROOT}/JointMovJ'
        else:
    ...
    def _build_speed_request(self, key: str, value: int):
        ...
        if key == 'speed_j':
            # V3 compatibility: V4 VelJ service is V3 SpeedJ.
            request = SpeedJ.Request()
            request.r = value
            return self._speed_j_client, request, 'SpeedJ', f'{SERVICE_ROOT}/SpeedJ'
        if key == 'acc_j':
            request = AccJ.Request()
            request.r = value
            return self._acc_j_client, request, 'AccJ', f'{SERVICE_ROOT}/AccJ'
        if key == 'speed_l':
            # V3 compatibility: V4 VelL service is V3 SpeedL.
            request = SpeedL.Request()
            request.r = value
            return self._speed_l_client, request, 'SpeedL', f'{SERVICE_ROOT}/SpeedL'
        if key == 'acc_l':
    ...
    def _tcp_callback(self, msg: ToolVectorActual) -> None:
        ...
        with self._lock:
            ...
            # V3 compatibility: the stock V3 feedback node publishes ToolVectorActual
            # but does not publish RobotStatus, so fresh TCP feedback is treated as a
            # dashboard connection hint and RobotMode polling supplies the mode/enabled state.
            if self._snapshot.connected is not True:
    ...
    def _handle_robot_mode(self, future) -> None:
        try:
            ...
            # V3 compatibility: RobotMode response field changed from robot_return to mode.
            raw_mode_text = self._clean_text(getattr(response, 'mode', getattr(response, 'robot_return', '')))
            ...
            self._log_event(
                'service',
                f'RobotMode -> {SERVICE_ROOT}/RobotMode {{}}: ok | mode={raw_mode_text!r}, res={response.res}',
            )
    ...
    def _handle_error_id(self, future) -> None:
        try:
            response = future.result()
            # V3 compatibility: GetErrorID response field changed from robot_return to error_id.
            raw_error_text = self._clean_text(getattr(response, 'error_id', getattr(response, 'robot_return', '')))
            ...
            self._log_event(
                'service',
                f'GetErrorID -> {SERVICE_ROOT}/GetErrorID {{}}: ok | error_id={raw_error_text!r}, res={response.res}',
            )
    ...
    def _build_stop_jog_request(self) -> MoveJog.Request:
        # V3 compatibility: V3 exposes MoveJog, not the V4 StopMoveJog service.
        # The V3 dobot_api implementation sends MoveJog() when axis_id is an empty string
        # and param_value is empty, which is the safest available jog-stop equivalent.
    ...
    def _apply_startup_tool_tcp_reset_if_needed(self) -> None:
        ...
        self._send_tool_tcp(
            ZERO_TOOL_TCP_VALUES,
            'Set Tool 1 to 0,0,0,0,0,0',
            allow_local_queue=False,
        )
