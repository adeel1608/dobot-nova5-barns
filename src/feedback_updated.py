...
class fankuis():
    ...
    def feed(self):
        """Receive and parse one feedback packet.

        Returns:
            ["NG"] on failure, otherwise a list whose first two elements remain:
              [tool_vector_actual(6,), q_target(6,), ...extra fields...]

            Extra fields appended (indices):
              2: q_actual(6,)
              3: qd_actual(6,)
              4: m_actual(6,)              # joint torque estimate (if provided by controller)
              5: actual_TCP_force(6,)       # TCP wrench estimate (if provided)
              6: six_force_online(int)      # 1 if 6-axis FT online, else 0
              7: six_force_value(6,)        # external/internal FT sensor values (if online)
              8: load(float)                # controller-reported load (if provided)
        """
        ...
        try:
            ...
            if hex((a['test_value'][0])) == '0x123456789abcdef':
                ...
                # Extra fields (may be zeros depending on firmware / hardware options)
                q_actual = a['q_actual'][0]
                qd_actual = a['qd_actual'][0]
                m_actual = a['m_actual'][0]
                tcp_force = a['actual_TCP_force'][0]
                six_online = int(a['six_force_online'][0])
                six_force = a['six_force_value'][0]
                load = float(a['load'][0])

                return [tool_v, tool_j, q_actual, qd_actual, m_actual, tcp_force, six_online, six_force, load]
            else:
