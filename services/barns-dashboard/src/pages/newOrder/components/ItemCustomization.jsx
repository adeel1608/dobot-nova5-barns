...
export default function ItemCustomization({
  ...
  useEffect(() => {
    ...
    // Get milk ingredient metadata (for base_units)
    const milkIngredient = item.selectedMenuItem.default_ingredients?.find(
      ing => ing.category === 'milk'
    );

    // Calculate milk volume using current milkAmount state (slider value, in grams)
    // so the bar updates whenever the user moves the milk slider
    let baseMilkAmount = 0;
    if (milkIngredient) {
      const milkBaseUnits = (milkIngredient.base_units || '').toLowerCase();
      if (milkBaseUnits === 'ml') {
        baseMilkAmount = milkAmount; // already ml
      } else {
        // Convert grams → ml using milk density
        baseMilkAmount = milkAmount / INGREDIENT_DENSITIES.milk;
      }
    }
        ...
        <div className="capacity-indicator-floating">
          ...
          <div className="capacity-bar-container">
            ...
            {(() => {
              ...
              // Override water volume with the live waterAmount state so the bar
              // responds when the user moves the water slider
              const waterDefaultIng = item.selectedMenuItem.default_ingredients?.find(ing => ing.category === 'water');
              if (waterDefaultIng) {
                const waterBaseUnits = (waterDefaultIng.base_units || '').toLowerCase();
                const waterVolume = waterBaseUnits === 'ml' ? waterAmount : waterAmount / (INGREDIENT_DENSITIES.water || 1.0);
                if (waterVolume > 0) {
                  defaultIngredientsByCategory['water'] = waterVolume;
                } else {
                  delete defaultIngredientsByCategory['water'];
                }
              }
                ...
                <div className="replaceable-options">
                  {ingredients.map(ing => (
                    <button
                      ...
                      className={`option-btn ${String(currentSelection.itemId) === String(ing.ingredient_id) ? 'active' : ''
                        }`}
                    >
                      {ing.name || ing.type}
