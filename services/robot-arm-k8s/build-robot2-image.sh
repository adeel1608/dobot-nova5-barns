#!/usr/bin/env bash
###############################
# BARNS Robot 2 Docker Image Build Script
...
IMAGE_NAME=${IMAGE_NAME:-"barns-robot2"}
...
log "Building Robot 2 Docker Image"
log "Project Root: $PROJECT_ROOT"
log "Image: ${IMAGE_NAME}:${IMAGE_TAG}"
echo

# Check if Dockerfile exists
if [ ! -f "${SCRIPT_DIR}/Dockerfile.robot2" ]; then
    error "Dockerfile.robot2 not found in ${SCRIPT_DIR}"
fi
...
if [ "$NO_CACHE" = true ]; then
    log "Starting Docker build for Robot 2 (NO CACHE - full rebuild)..."
    log "This may take 15-30 minutes depending on your system..."
else
    log "Starting Docker build for Robot 2 (cached)..."
...
DOCKER_BUILD_ARGS=(
    -f "${SCRIPT_DIR}/Dockerfile.robot2"
...
if [ $? -eq 0 ]; then
    ...
    echo "    -e IP_ADDRESS=192.168.200.248 \\"
